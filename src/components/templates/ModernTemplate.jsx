  ...
  return (
    <div style={{ marginBottom: '14px' }}>
      <h2 style={{
        ...
        color: theme.accent, margin: '0 0 6px 0',
      }}>
        {children}
      </h2>
...
function EntryBlock({ children, theme }) {
  return (
    <div style={{
      paddingLeft: '14px',
      borderLeft: `2.5px solid ${theme.accentLight}`,
      marginBottom: '18px',
    }}>
      {children}
    </div>
...
export function ModernTemplate({ cv, theme }) {
  ...
  return (
    <div style={{
      ...
      color: '#1a1a2e', background: '#ffffff', minHeight: '297mm',
    }}>
            ...
            {cv.experience.map(exp => (
              <EntryBlock key={exp.id} theme={t}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <span style={{ fontSize: '10px', color: '#aaa', whiteSpace: 'nowrap', marginLeft: '12px', flexShrink: 0 }}>
                    {dateRange(exp.start, exp.end, exp.current)}
                  </span>
                </div>
                {exp.description && (
                  <div style={{ fontSize: '11.5px', color: '#555', marginTop: '7px', lineHeight: 1.68, whiteSpace: 'pre-line' }}>
                    {exp.description}
                  </div>
                )}
              </EntryBlock>
